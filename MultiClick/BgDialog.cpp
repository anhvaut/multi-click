...
CBgDialog::CBgDialog(CWnd* pParent /*=NULL*/)
{
	//{{AFX_DATA_INIT(CBgDialog)
...
CBgDialog::CBgDialog(UINT uResourceID, CWnd* pParent)
...
CBgDialog::CBgDialog(LPCTSTR pszResourceID, CWnd* pParent)
...
void CBgDialog::FreeResources(BOOL bCheckForNULL)
{
...
void CBgDialog::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// EasyMove mode
...
void CBgDialog::OnSize(UINT nType, int cx, int cy) 
{
...
BOOL CBgDialog::OnEraseBkgnd(CDC* pDC) 
{
	CRect		rWnd;
	int			nX			= 0;
	int			nY			= 0;
...
void CBgDialog::OnPostEraseBkgnd(CDC* pDC)
{
...
//
#ifndef UNDER_CE
HRGN CBgDialog::ScanRegion(HBITMAP hBitmap, BYTE byTransR, BYTE byTransG, BYTE byTransB)
{
...
//
#ifndef UNDER_CE
LPBYTE CBgDialog::Get24BitPixels(HBITMAP hBitmap, LPDWORD lpdwWidth, LPDWORD lpdwHeight)
{
...
//
DWORD CBgDialog::ActivateEasyMoveMode(BOOL bActivate)
{
...
//
DWORD CBgDialog::SetBitmap(int nBitmap, COLORREF crTransColor)
{
...
//			Failed creating region
//
DWORD CBgDialog::SetBitmap(HBITMAP hBitmap, COLORREF crTransColor)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//			Invalid mode.
//
DWORD CBgDialog::SetMode(BYTE byMode, BOOL bRepaint)
{
...
#ifndef UNDER_CE
DWORD CBgDialog::ShrinkToFit(BOOL bRepaint)
{
	CRect	rWnd;
	CRect	rClient;
	DWORD	dwDiffCX;
