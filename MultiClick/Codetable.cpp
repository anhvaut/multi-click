...
int CCodeTable::isVietNamese(CHAR* st){
//	char buffer[128];
	WCHAR c= 0x1ea3;
	 ...
	 for(int m=0;m<len;m++){
		...
		for(int i=0;i<12;i++){
			for(int j=0;j<6;j++){
					if(st[m]==_8bitVowelL[0][i][j]&&st[m]!='a'&&st[m]!='e'&&st[m]!='u'&&st[m]!='i'&&st[m]!='o'&&st[m]!='i'&&st[m]!='y'){
						 //wsprintf(buffer, "TextHook i=%d|j=%d",i,j);
...
int CCodeTable::isVietNameseW(WORD* st,int len){
	//char buffer[128];
	 ...
	 for(int m=0;m<len;m++){
			...
			for(int j=0;j<6;j++){
				for(int k=0;k<2;k++){
					if(st[m]==_16bitVowelL[k][i][j]&&st[m]!='a'&&st[m]!='e'&&st[m]!='u'&&st[m]!='i'&&st[m]!='o'&&st[m]!='i'&&st[m]!='y'){
						 //wsprintf(buffer, "TextHook %d|i=%d|j=%d", k,i,j);
						//MessageBox(NULL, buffer,"ERROR_CAPTION", MB_OK);
						
						return 1;
					}else if(st[m]==_16bitVowelH[k][i][j]&&st[m]!='a'&&st[m]!='e'&&st[m]!='u'&&st[m]!='i'&&st[m]!='o'&&st[m]!='i'&&st[m]!='y'){
						return 2;
					}
				}
			}
		...
		for(i=0;i<1;i++)
		if(st[m]==_16bitDoubleL[i]) return 1;

	 }//end for
	return 0;
}
CHAR* CCodeTable::convert(WORD *st,int len,int type){
	CHAR pChar[257];
	int s;
	 for(int m=0;m<len;m++){
...
WORD *putUnicodeCharUtf8(WORD *buf, WORD ch, int & len)
{
	if (ch < 0x0080) {
		len = 1;
		*buf++ = (unsigned char)ch;
		...
		len = 2;
		*buf++ = (0xC0 | (CHAR)(ch >> 6));
		*buf++ = (0x80 | (CHAR)(ch & 0x003F));
	} else {
		len = 3;
		*buf++ = (0xE0 | (CHAR)(ch >> 12));
		*buf++ = (0x80 | (CHAR)((ch >> 6) & 0x003F));
		*buf++ = (0x80 | (CHAR)(ch & 0x003F));
	}
	return buf;	
}
...
CHAR* CCodeTable::convert2Utf8(WORD *st,int len,int type){
...
void CCodeTable::Convert_String(CHAR *string_out, wchar_t *string_in)

{
    ...
    while (i <scnt) 
        { 
			if (string_in[i] < 0x0080){
				*string_out =(CHAR) string_in[i++];
			}
		    else if (string_in[i] < 0x0800)
            {

        // 2byte

				utf8 = (0xC0 | (CHAR)(string_in[i] >> 6));

				utf8 += (0x80 | (CHAR)(string_in[i] & 0x003F));;
				...
				*string_out= utf8;
				...
				utf8 = (string_in[i++] & 0x0f)<<12;

				utf8 += ((string_in[i++] & 0x3f)<<6);

				utf8 += (string_in[i++] & 0x3f);
