...
BEGIN_MESSAGE_MAP(CustomBtn, CButton)
    //{{AFX_MSG_MAP(CustomBtn)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CustomBtn::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CustomBtn::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CustomBtn::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
HBRUSH CustomBtn::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CustomBtn::OnSysColorChange() 
{
...
LRESULT CustomBtn::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CustomBtn::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CustomBtn::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CustomBtn::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CustomBtn::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CustomBtn::OnEnable(BOOL bEnable) 
{
...
void CustomBtn::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CustomBtn::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CustomBtn::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CustomBtn::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CustomBtn::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CustomBtn::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CustomBtn::OnClicked() 
{	
	SetFocus();
...
void CustomBtn::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
void CustomBtn::PaintBk(CDC* pDC)
{
...
HBITMAP CustomBtn::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CustomBtn::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CustomBtn::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CustomBtn::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CustomBtn::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HBITMAP CustomBtn::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
...
//		If the function fails, the return value is NULL.
//
HBITMAP CustomBtn::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
	HBITMAP		hGrayBitmap = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON CustomBtn::CreateGrayscaleIcon(HICON hIcon)
{
...
//		If the function fails, the return value is NULL.
//
HICON CustomBtn::CreateDarkerIcon(HICON hIcon)
{
	HICON		hGrayIcon = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	BITMAP		bmp;
...
COLORREF CustomBtn::DarkenColor(COLORREF crColor, double dFactor)
{
...
//
DWORD CustomBtn::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CustomBtn::SetIcon(int nIconIn, int nIconOut)
{
...
//			Failed loading the specified resource.
//
DWORD CustomBtn::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CustomBtn::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CustomBtn::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
...
//				By default, CustomBtn buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CustomBtn::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CustomBtn buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CustomBtn::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CustomBtn buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CustomBtn::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CustomBtn::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CustomBtn::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CustomBtn::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CustomBtn::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CustomBtn::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CustomBtn::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CustomBtn::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CustomBtn::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CustomBtn::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CustomBtn::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CustomBtn::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CustomBtn::ActivateTooltip(BOOL bActivate)
{
...
//			Function executed successfully.
//
DWORD CustomBtn::EnableBalloonTooltip()
{
...
//				If TRUE the control will be repainted.
//
void CustomBtn::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CustomBtn::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CustomBtn::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CustomBtn::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
//			Function executed successfully.
//
DWORD CustomBtn::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
#ifdef	BTNST_USE_SOUND
DWORD CustomBtn::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
...
// This is a virtual function that can be rewritten in CustomBtn-derived classes
...
//			Function executed successfully.
//
DWORD CustomBtn::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CustomBtn::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	// Draw pressed button
	if (m_bIsPressed)
	{
