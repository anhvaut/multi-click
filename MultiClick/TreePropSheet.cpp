...
BEGIN_MESSAGE_MAP(CTreePropSheet, CPropertySheet)
	...
	ON_COMMAND(IDOK,TaskBar)
	ON_COMMAND(ID_APPLY_NOW,openHelp)
END_MESSAGE_MAP()
...
//	m_btnOk.EnableWindow(1);
	//m_btnOk.ShowWindow(SW_NORMAL);

	//CustomBtn* pApplyButton = GetDlgItem (ID_APPLY_NOW);
 //  ASSERT (pApplyButton);
 //  pApplyButton->ShowWindow (SW_NORMAL);
	

	//m_btnOk.DrawTransparent();

	changeTreePropSheetGUI(m_hWnd,regedit.guiLanguage);
...
//	CWnd* pApplyButton = GetDlgItem (ID_APPLY_NOW);
 ...
 //  pApplyButton->ShowWindow (SW_NORMAL);
	...
	GetDlgItem(ID_APPLY_NOW)->EnableWindow(1);
...
void CTreePropSheet::TaskBar(){
	
	
		EndDialog(1);
		//DestroyWindow(main_hWnd);
		Stray.OnEndKey1(main_hWnd);
...
void CTreePropSheet::setHWND(HWND mainHwnd){
 main_hWnd=mainHwnd;
}
